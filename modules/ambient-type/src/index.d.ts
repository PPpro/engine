// default ambient types
/// <reference path="../lib/consts.d.ts"/>
/// <reference path="./types/editor-extends.d.ts"/>
/// <reference path="./types/globals.d.ts"/>
/// <reference path="./types/jsb.d.ts"/>
/// <reference path="./types/webGL.extras.d.ts"/>
/// <reference path="./types/webGL2.extras.d.ts"/>

// node_modules type
/// <reference path="../../../node_modules/@cocos/cannon/CANNON.d.ts"/>
/// <reference path="../../../node_modules/@types/jest/index.d.ts"/>

// pal types
/// <reference path="./types/pal/audio.d.ts"/>
/// <reference path="./types/pal/env.d.ts"/>
/// <reference path="./types/pal/input-box.d.ts"/>
/// <reference path="./types/pal/input.d.ts"/>
/// <reference path="./types/pal/minigame.d.ts"/>
/// <reference path="./types/pal/pacer.d.ts"/>
/// <reference path="./types/pal/screen-adapter.d.ts"/>
/// <reference path="./types/pal/system-info.d.ts"/>
/// <reference path="./types/pal/wasm.d.ts"/>

// external types
/// <reference path="../../../native/external/emscripten/external-wasm.d.ts"/>
/// <reference path="../../../native/external/emscripten/bullet/bullet.d.ts"/>
/// <reference path="../../../native/external/emscripten/physx/physx.d.ts"/>
/// <reference path="../../../native/external/emscripten/webgpu/webgpu.d.ts"/>
/// <reference path="../../../native/external/emscripten/spine/spine.d.ts"/>
