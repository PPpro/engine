...
for file in $ts_files; do
  ...
  npx eslint "$file" --plugin import --parser @typescript-eslint/parser --fix --rule 'import/order: [error,{ groups: [['builtin', 'external', 'internal']] }]' --no-eslintrc # 使用你的 ESLint 配置文件和选项
done
